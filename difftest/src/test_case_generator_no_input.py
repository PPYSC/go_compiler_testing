...
class TestCaseGenerator:
    def __init__(self, seed_path, test_unit_path, go_generator, tokenizer):
        ...
        self.cmdrunnerlist = [CmdRunner("/bigdata/qiuhan/go1.23rc2/bin/go build -o exe0", "./exe0"), CmdRunner("/bigdata/qiuhan/go1.22.5/bin/go build -o exe1", "./exe1")]
    ...
    def _create_test_unit(self, code, path, i):
    ...
    def _execute_test_unit(self, code, index):
        dir_path = os.path.join(self.test_unit_path, str(index))
        self._create_test_unit(code, dir_path, index)
    ...
    def generate_test_case_loop(self):
        with tqdm() as pbar:
            ...
            while True:
                ...
                if not flag:
                    check = self._execute_test_unit(new_code, index)
                    if check == -1:
                        d_count += 1
