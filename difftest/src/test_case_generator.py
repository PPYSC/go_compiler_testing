...
class TestCaseGenerator:
    def __init__(self, seed_path, test_unit_path, go_generator, tokenizer):
        ...
        self.seed_corpus = data_from_jsonl(self.seed_path)
    ...
    def _test_case_do_filter(self, code):
        flag = False
        count = {"has_error": 0, "undefined_behavior": 0}
    ...
    def generate_test_case_loop(self):
        with tqdm() as pbar:
            ...
            while True:
                ...
                flag, count = self._test_case_do_filter(new_code)
                if not flag:
                    dir_path = os.path.join(self.test_unit_path, str(index))
                    self._create_test_unit(new_code, dir_path, index)
